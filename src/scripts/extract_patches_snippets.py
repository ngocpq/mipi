    ...
    for f in listdirs:
        ...
        if os.path.isfile(os.path.join(patches_dir, f)):
            if fnmatch.fnmatch(f, pattern):
                ...
                if "Dcorrect" in filepath:
                    patchId += "_correct"
                elif "Doverfitting" in filepath:
                ...
                try:
                    ...
                    sucess_logger.info(
                        '%s | %s | %s | %s | %s' % (filepath, projectId, projectId + bugId, toolId, toolId + patchId))
